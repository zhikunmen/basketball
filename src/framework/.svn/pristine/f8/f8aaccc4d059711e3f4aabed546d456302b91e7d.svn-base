...
class SoundUtils {
    ...
    public playSound(id: string, loop: number = 1, onPlayComplete?: () => void): void {
        let cfg = Config.getConfig(SoundConfig);
        let config: SoundConfig = cfg.get(id);
        if (config) {
     ...
     * force 强制设
     */
    public setBGMValume(volume: number,force:boolean = false): void {
        if(force){
        }else{
    ...
    public restartBGM():void{
        let soundBGM:number = Number(egret.localStorage.getItem('soundBGM'));
