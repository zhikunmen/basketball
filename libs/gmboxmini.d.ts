...
declare namespace gmbox {
    ...
    /**
     * UpdateManager 对象，用来管理更新，可通过 gmbox.getUpdateManager 接口获取实例。
     */
    type UpdateManager = {
        /**
    ...
    */
    ...
    interface BannerAd {
        ...
        style: {
            ...
            /** banner 广告组件的左上角纵坐标*/
            top: number,
            /** banner 广告组件的宽度。最小 300，最大至 屏幕宽度（屏幕宽度可以通过 gmbox.getSystemInfoSync() 获取）。*/
            width: number,
            /** banner 广告组件的高度*/
            height: number,
    ...
    /**
     * 创建激励视频广告组件。请通过 gmbox.getSystemInfoSync() 返回对象的 SDKVersion 判断基础库版本号 >= 2.0.4 后再使用该 API。同时，开发者工具上暂不支持调试该 API，请直接在真机上进行调试。
     */
    function createRewardedVideoAd(res: { adUnitId: string }): RewardedVideoAd;

    /**
     * 创建 banner 广告组件。请通过 gmbox.getSystemInfoSync() 返回对象的 SDKVersion 判断基础库版本号 >= 2.0.4 后再使用该 API。同时，开发者工具上暂不支持调试该 API，请直接在真机上进行调试。
     */
    ...
    /** 显示 loading 提示框。需主动调用 gmbox.hideLoading 才能关闭提示框*/
    ...
    */
    ...
    /**
     * 通过 WebSocket 连接发送数据，需要先 gmbox.connectSocket，并在 gmbox.onSocketOpen 回调之后才能发送。
     */
    ...
    */
    ...
    /**
     * InnerAudioContext 实例，可通过 gmbox.createInnerAudioContext 接口获取实例。
     */
    ...
    /**
     * 通过 gmbox.login 接口获得的用户登录态拥有一定的时效性。用户越久未使用小程序，用户登录态越有可能失效。反之如果用户一直在使用小程序，则用户登录态一直保持有效。具体时效逻辑由微信维护，对开发者透明。开发者只需要调用 gmbox.checkSession 接口检测当前用户登录态是否有效。登录态过期后开发者可以再调用 gmbox.login 获取新的用户登录态。
     */
    ...
    interface AuthSetting {
        /** 是否授权用户信息，对应接口 gmbox.getUserInfo*/
        userInfo: boolean;
        /** 是否授权地理位置，对应接口 gmbox.getLocation*/
        userLocation: boolean;
        /** 是否授权微信运动步数，对应接口 gmbox.getWeRunData*/
        werun: boolean;
        /** 是否授权保存到相册 gmbox.saveImageToPhotosAlbum*/
    ...
    /** 获取用户过去三十天微信运动步数。需要先调用 gmbox.login 接口。步数信息会在用户主动进入小程序时更新。*/
    ...
    /** 监听加速度数据事件。频率根据 gmbox.startAccelerometer() 的 interval 参数。可使用 gmbox.stopAccelerometer() 停止监听。*/
    ...
    /**
     * 监听罗盘数据，频率：5 次/秒，接口调用后会自动开始监听，可使用 gmbox.stopCompass 停止监听。
    ...
    /**
     * 监听设备方向变化事件。频率根据 gmbox.startDeviceMotionListening() 的 interval 参数。可以使用 gmbox.stopDeviceMotionListening() 停止监听。
     */
...
//      */
//     gmboxBindCanvasTexture: (texture: number, canvas: gmbox.Canvas) => void
// }
