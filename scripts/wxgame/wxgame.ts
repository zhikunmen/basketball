    ...
    async onFile(file: plugins.File) {
        if (file.extname == '.js') {
            ...
            if (filename == 'libs/modules/egret/egret.js' || filename == 'libs/modules/egret/egret.min.js') {
                ...
                if (filename == 'libs/modules/matter/matter.js' || filename == 'libs/modules/matter/matter.min.js') {
                    content += ';window.Matter = matter';
                } 
                if(filename == 'libs/tweenMax/tweenMax.js' || filename == 'libs/tweenMax/tweenMax.min.js'){
                    content += ';Object.assign(window,window.global);';
                }
                if (filename == 'main.js') {
                    var timestramp:number = Date.now();
                    content = content.replace(/egret/g,"egret" + timestramp);
                    content = "var egret"+ timestramp +" = window.egret;" + content;
                }
                content = "var egret = window.egret;" + content;
                if (filename == 'main.js') {
                    content += ";window.Main = Main;"
                    content += ";window.EventUtils = EventUtils;"
                }
                file.contents = new Buffer(content);
            }
        }
